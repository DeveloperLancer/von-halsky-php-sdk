...
/** GPSR person or legal entity responsible for a product in the EU. */
final class ResponsiblePerson implements RequestDtoInterface
{
    public function __construct(
        ...
        if ($name !== null) {
            RequestValidator::stringLength($name, 0, 500, 'ResponsiblePerson.name');
        }
        if ($email !== null) {
            self::validateEmail($email, 'ResponsiblePerson.email');
        }
        if ($phone !== null) {
            self::validatePhone($phone, 'ResponsiblePerson.phone');
        }
        if ($address !== null && $address->building === null) {
            throw new InvalidRequestException('ResponsiblePerson.address.building', 'is required');
        }
        if ($unstructuredAddress !== null) {
            RequestValidator::stringLength($unstructuredAddress, 0, 300, 'ResponsiblePerson.unstructuredAddress');
        }
    }
